      ...
      <div className="space-y-3 pb-4">
          ...
          {isInstalled ? (
            ...
            <div className="space-y-3">
              <p className="text-xs text-stone-400 leading-relaxed">
                Install ArthaLedger on your home screen for quick offline access — works like a native app.
              </p>
              <button
                onClick={handleInstallClick}
                className="w-full flex items-center justify-center gap-2 rounded-xl bg-amber-600 px-4 py-3 text-sm font-semibold text-white hover:bg-amber-700 transition-colors shadow-sm cursor-pointer"
              >
              ...
              {(!installPrompt || showInstructions) && (
                <div className="rounded-xl border border-amber-100 bg-amber-50/20 p-3 space-y-2 text-xs">
                  <p className="font-bold text-stone-700">How to Install manually:</p>
                  <div className="space-y-1.5 text-stone-600">
                    <p> <span className="font-semibold text-stone-800">iOS (Safari):</span> Tap the <strong className="text-amber-800">Share</strong> button in Safari, scroll down and select <strong className="text-amber-800">"Add to Home Screen"</strong>.</p>
                    <p> <span className="font-semibold text-stone-800">Android (Chrome):</span> Tap the <strong className="text-amber-800">⋮ Menu</strong> button in Chrome and select <strong className="text-amber-800">"Add to Home Screen"</strong> or <strong className="text-amber-800">"Install app"</strong>.</p>
                    <p> <span className="font-semibold text-stone-800">Desktop (Chrome/Edge):</span> Click the <strong className="text-amber-800">Install icon</strong> in the address bar.</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <motion.section initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.07 }}
          ...
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-stone-400">Push Notifications</h2>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-stone-700 leading-tight">Budget & Threshold Alerts</p>
              <p className="text-xs text-stone-400 mt-0.5 leading-relaxed">
                Receive instant system alerts when category spending reaches 80% warning or 100% hard limit.
              </p>
            </div>
          </div>
          ...
          {notifPermission === 'granted' ? (
            ...
            <div className="rounded-xl border border-red-100 bg-red-50/30 p-2.5 text-xs text-red-700">
              ⚠️ Notifications are blocked in your browser settings. Please enable notification permissions in your browser bar.
            </div>
          ) : (
            <button
              type="button"
              onClick={async () => {
                ...
                if (perm === 'granted') {
                  ...
                  sendNativeNotification('🔔 Notifications Enabled!', {
                    body: 'You will receive system alerts when category spending reaches budget thresholds.',
                  })
                } else if (perm === 'denied') {
                  showToast('Notifications blocked in browser settings', 'error')
                }
              }}
              className="w-full flex items-center justify-center gap-2 rounded-xl bg-amber-600 px-4 py-2.5 text-xs font-bold text-white hover:bg-amber-700 transition-colors shadow-xs cursor-pointer"
            >
              <Bell size={14} /> Enable Push Notifications
            </button>
          )}

          {/* Daily Evening Reminder Toggle & Time Selector */}
          <div className="pt-3 border-t border-amber-100/60 space-y-2.5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Clock size={14} className="text-amber-600" />
                <div>
                  <p className="text-xs font-bold text-stone-700">Daily Evening Reminder</p>
                  <p className="text-[11px] text-stone-400">Receive a system prompt to log today's expenses</p>
                </div>
              </div>
              <Toggle
                ...
                onChange={async () => {
                  if (notifPermission !== 'granted') {
                    ...
                    setNotifPermission(perm)
                    if (perm !== 'granted') {
                      showToast('Notification permission required', 'error')
                      return
                    }
                  }
                  const next = !dailyReminder.enabled
                  setDailyReminder((prev) => ({ ...prev, enabled: next }))
                  saveDailyReminderSettings({ enabled: next, time: dailyReminder.time })
                  showToast(next ? `Daily Evening Reminder enabled (${dailyReminder.time}) ✓` : 'Daily reminder disabled')
                }}
              />
            </div>
            ...
            {dailyReminder.enabled && (
              <div className="space-y-2.5 pt-1">
                <div className="rounded-xl bg-amber-50/50 border border-amber-100 p-3 text-xs space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-stone-700 font-bold">Scheduled Reminder Times:</span>
                    <button
                      type="button"
                      onClick={() => {
                        const times = dailyReminder.times || [dailyReminder.time || '19:00']
                        const newTimes = [...times, '12:00']
                        setDailyReminder((prev) => ({ ...prev, times: newTimes, time: newTimes[0] }))
                        saveDailyReminderSettings({ enabled: true, times: newTimes })
                        showToast('Added new reminder time ✓')
                      }}
                      className="flex items-center gap-1 rounded-lg bg-amber-600 px-2 py-1 text-[11px] font-bold text-white hover:bg-amber-700 transition-colors cursor-pointer"
                    >
                  ...
                  <div className="space-y-2">
                    {(dailyReminder.times || [dailyReminder.time || '19:00']).map((t, idx) => (
                      <div key={idx} className="flex items-center justify-between gap-2 bg-white rounded-lg p-2 border border-amber-200">
                        <div className="flex items-center gap-2">
                          <Clock size={12} className="text-amber-600" />
                          <span className="text-xs font-semibold text-stone-700">Reminder #{idx + 1}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <input
                            type="time"
                            value={t}
                            onChange={(e) => {
                              const newT = e.target.value || '19:00'
                              const currentTimes = [...(dailyReminder.times || [dailyReminder.time || '19:00'])]
                              currentTimes[idx] = newT
                              setDailyReminder((prev) => ({ ...prev, times: currentTimes, time: currentTimes[0] }))
                              saveDailyReminderSettings({ enabled: true, times: currentTimes })
                              showToast(`Reminder #${idx + 1} set to ${newT} ✓`)
                            }}
                            className="rounded-md border border-amber-300 bg-amber-50/30 px-2 py-0.5 text-xs text-stone-800 font-bold outline-none cursor-pointer"
                          />
                          {(dailyReminder.times || [dailyReminder.time || '19:00']).length > 1 && (
                            <button
                              type="button"
                              onClick={() => {
                                const currentTimes = (dailyReminder.times || [dailyReminder.time || '19:00']).filter((_, i) => i !== idx)
                                setDailyReminder((prev) => ({ ...prev, times: currentTimes, time: currentTimes[0] }))
                                saveDailyReminderSettings({ enabled: true, times: currentTimes })
                                showToast(`Removed reminder time ✓`)
                              }}
        ...
        <motion.section initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }}
          ...
          <button onClick={() => setShowDownloadModal(true)}
            className="w-full flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50/60 px-3 py-2.5 text-sm text-stone-700 hover:bg-amber-100 transition-colors">
            <div className="rounded-lg bg-amber-100 p-1.5"><Download size={13} className="text-amber-700" /></div>
            <div className="flex-1 text-left">
              <p className="text-sm font-medium text-stone-700 leading-tight">Download Report</p>
              <p className="text-[10px] text-stone-400 mt-0.5">PDF or spreadsheet · filter by project, book & date</p>
            </div>
