...
export default function AnalyticsPage() {
  const { entriesByBook, booksByProject, projects, categories = [] } = useAppData()
  ...
  // ── Current-month spend per category & subcategory (for budget tracking) ──
  const spentByCategory = useMemo(() => {
    ...
    allEntries.forEach(e => {
      ...
      if (e.timestamp < startOfMonth) return
      const cat = e.category || 'General'
      const sub = e.subcategory
      const amt = Number(e.amount || 0)
      result[cat] = (result[cat] || 0) + amt
      if (sub) {
        result[sub] = (result[sub] || 0) + amt
        result[`${cat} · ${sub}`] = (result[`${cat} · ${sub}`] || 0) + amt
      }
    })
    return result
  }, [allEntries])

  // ── All known categories & subcategories (union of budgeted + actual + saved) ──
    ...
    allEntries.forEach(e => {
      if (e.type === 'expense' && e.category) {
        ...
        if (e.subcategory) {
          ...
          set.add(`${e.category} · ${e.subcategory}`)
        }
      }
    })
    Object.keys(budgets).forEach(bKey => {
      if (set.has(bKey) || (spentByCategory[bKey] || 0) > 0) {
        set.add(bKey)
      }
    })
    return Array.from(set).sort()
  }, [allEntries, budgets, categories, spentByCategory])
  ...
  // Expense by Main Category — clean breakdown
  ...
  // Get available subcategories for the currently selected main category
  const availableSubcatFilters = useMemo(() => {
    if (!selectedCategory) return []
    const set = new Set()
    spendingEntries.forEach(e => {
      if (e.type === 'expense' && (e.category || 'General') === selectedCategory && e.subcategory) {
        set.add(e.subcategory)
      }
    })
    return Array.from(set).sort()
  }, [spendingEntries, selectedCategory])

  // Get filtered transactions for the selected category & subcategory filter
  ...
  }, [spendingEntries, selectedCategory, selectedSubcatFilter])
      ...
      <div className="space-y-3 pb-6">
        ...
        {isEmpty ? (
          ...
          <>
            ...
            <BudgetSection
              budgets={budgets}
              allExpenseEntries={allEntries.filter(e => !e.isTransfer && e.type === 'expense')}
              categories={categories}
              sym={sym}
              onSetBudget={(cat, config) => setBudget(cat, config)}
              onDeleteBudget={(cat) => setBudget(cat, { soft: 0 })}
              bookIdMap={bookIdMap}
            />

            {/* ── Spending breakdown ── */}
            {byCategory.length > 0 && (              <motion.section
        ...
        {showCategoryModal && selectedCategory && (
          ...
          >
            ...
            >
              ...
              {/* Subcategory Filter Chips inside Category Modal */}
              {availableSubcatFilters.length > 0 && (
                <div className="mb-4 space-y-1.5">
                  <span className="text-[10px] font-bold text-stone-400 uppercase tracking-wider block">
                    Filter Subcategories
                  </span>
                  <div className="flex items-center gap-1.5 overflow-x-auto pb-1.5 snap-x">
                    <button
                      type="button"
                      onClick={() => setSelectedSubcatFilter('all')}
                      className={`snap-start rounded-full border px-3 py-1 text-xs font-medium transition-all ${
                    ...
                    {availableSubcatFilters.map((sub) => (
                      <button
                        key={sub}
                        type="button"
                        onClick={() => setSelectedSubcatFilter(sub)}
                        className={`snap-start rounded-full border px-3 py-1 text-xs font-medium transition-all ${
                          selectedSubcatFilter === sub
                            ? 'border-amber-500 bg-amber-100 text-amber-900 font-bold shadow-2xs'
                            : 'border-amber-200/80 bg-amber-50/40 text-stone-700 hover:bg-amber-50'
                        }`}
                      >
                        {sub}
                      </button>
